<link href="https://fonts.googleapis.com/icon?family=Material+Icons" rel="stylesheet">
<!-- BEGIN: VENDOR CSS-->
<link rel="stylesheet" type="text/css"
    href="app-assets/vendors/vendors.min.css">
<!-- END: VENDOR CSS-->
<link rel="stylesheet" type="text/css"
    href="app-assets/css/vendor-styles.css" id="vendor-styles">
@yield('vendor-styles')
<!-- BEGIN: Page Level CSS-->
<link rel="stylesheet" type="text/css"
    href="app-assets/css/themes/vertical-menu-nav-dark-template/materialize.min.css">
<link rel="stylesheet" type="text/css"
    href="app-assets/css/themes/vertical-menu-nav-dark-template/style.min.css">
<link rel="stylesheet" type="text/css"
    href="app-assets/css/pages/dashboard.min.css">
<!-- END: Page Level CSS-->
<link rel="stylesheet" type="text/css"
    href="app-assets/css/page-styles.css" id="page-styles">
@yield('page-styles')
<!-- BEGIN: Custom CSS-->
<link rel="stylesheet" type="text/css" href="app-assets/css/custom/custom.min.css">
<!-- END: Custom CSS-->

<!-- Scripts -->
<script src="js/app.js" defer></script>

